...
#include <math.h>
#include <string>
#include "ComprehensiveBench.decl.h"
...
class Main: public CBase_Main {
	...
	void finish(){
			...
			double iterationTimeSum = 0;
			CkPrintf("Iteration times "); for(int i = 0; i < numberOfIterations; i++) CkPrintf("%.1f ", iterationTimes[i]);
			for(int i = 0; i < numberOfIterations; i++) iterationTimeSum += iterationTimes[i];
			CkPrintf("\nIterations total time %.1f\n", iterationTimeSum);
		#endif
			...
			if(numLBCalls > 0){
				...
				for(int i = 0; i < numLBCalls; i++) LBTimeSum += LBCallTimes[i];
				CkPrintf("\nLB total time %.1f\n", LBTimeSum);
			}
		#endif

		//print ComprehensiveBench time
		CkPrintf("ComprehensiveBench time %.1f\n", iterationTimeSum + LBTimeSum);
...
#include "ComprehensiveBench.def.h"
