...
parse_pair <- function(path, iterations, first, after){
    ...
    lb <- parse_lb(path, runs)
    hw <- parse_hw(path, runs)
    comm <- parse_communication(path, runs)
...
lb_test <- function(){
    lbt <- "lb_test"
    lbt_path <- paste(path, lbt, sep="/")
    iterations <- 49
    first <- 9
    after <- c(10, 20, 30, 40)
    parse_benchmark(lbt_path, lbs, iterations, first, after)
...
stencil <- function(){
    stencil <- "stencil4d"
    stencil_path <- paste(path, stencil, sep="/")
    iterations <- 45
    first <- 9
    after <- c(10, 19, 28, 37)
    parse_benchmark(stencil_path, lbs, iterations, first, after)

}

kneighbor <- function(){
    kn_name <- "kneighbor"
    kn_paths <- paste(path, kn_name, sep="/")
    iterations <- 50
    first <- 10
    after <- c(11, 21, 31, 41)
    apply(as.array(kn_paths), 1, parse_benchmark, lbs = lbs, iterations = iterations, first = first, after = after)

}

write_csvs <- function(l, k, lb, st, path_results){
    write.csv( l, file = paste( path_results, "leanmd.csv", sep="/" ) )
    write.csv( k, file = paste( path_results, "kneighbor.csv", sep="/" ) )
    write.csv( st, file = paste( path_results, "stencil.csv", sep="/" ) )
    write.csv( lb, file = paste( path_results, "lb_test.csv", sep="/" ) )

}

lbs <- c("StatsLB", "HwTopoLB", "GreedyCommLB", "ScotchLB", "RefineCommLB")
path <- "/home/user/Results/machine/date/results/parsed"
...
write_csvs(l, k, lb, st, path_results)
