  ...
  public:
    Main(CkArgMsg *m) {
      ...
      if (m->argc!=5 && m->argc!=6) {
      	CkPrintf("Usage: %s <#elements> <#iterations> <msg size> <amount of work> [ldb freq]\n", m->argv[0]);
      	delete m;
      	CkExit();
      }
      ...
      amt_work = atoi(m->argv[4]);
      gLBFreq = 100000;
            if(m->argc==6) {
      gLBFreq = atoi(m->argv[5]);
            }
    ...
    void terminate(CkReductionMsg *msg) {
     ...
     CkPrintf("Steps time:\n");
     for (int i=0; i<numSteps; i++) CkPrintf("Iter %d:\t%f\n",i,timeRec[i]);
...
class Block: public CBase_Block {
  ...
  public:
    Block() {
      ...
      for (int i=1; i<= STRIDEK; i++, nidx++) {
        int tmpnei = thisIndex - (i * GAP);
    ...
    void startInternalIteration() {
      ...
      // int N = ((thisIndex * curIterMsgSize) / (32 * num_chares));
      // for (int i=0; i<N; i++)
      //   for (int j=0; j<N; j++) {
      //     sum += (thisIndex * i + j);
      //   }
